...
$view['slots']->set('title', $view['translator']->trans("My Pages / Groups"));
?>
<div class="col-md-9">
    <?php if($check && $check->getTokenFacebook()){ ?><form method="POST" action=""><?php } ?>
        <button type="<?php if($check && $check->getTokenFacebook()){ echo "submit"; } ?>" class="btn btn-success <?php if(!$check || ($check && !$check->getTokenFacebook())){ echo "disabled"; } ?>">
            <i class="fa fa-refresh"></i> <?php echo $view['translator']->trans("Re-Author Pages/Groups"); ?>
        </button>
    <?php if($check && $check->getTokenFacebook()){ ?></form><?php } ?>
    <hr class="mb5 mt2">
    <div class="panel panel-blue">
        <div class="panel-heading">
            <h2 class="panel-title">
                <i class="fa fa-archive"></i> <?php echo $view['translator']->trans("My Pages / Groups"); ?>
            </h2>
        </div>
                ...
                <tbody>
                    <?php if ($list) { ?>
                        ...
                        foreach ($list as $value) {
                            $url = $view['router']->generate('_del_page', array('id' => $value->getId()), true);
                            ?>
                            <tr>
    ...
    <div class="col-lg-12">
        <div class="row">
            <?php
            if ($page["NextPage"] > 1) {
                $urlNext = $view['router']->generate('_my_page', array(), true);
                ?>
                <ul class="pagination">
